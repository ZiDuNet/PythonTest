...
# 页面下滑到底加载列表
driver.get('http://www.ghmba.online/my/course/178')
...
for tag in soup.find_all('li', class_='task-item task-content mouse-control infinite-item color-gray bg-gray-lighter'):
    ...
    for knameurl2 in knameurl:
        ...
        for tag2 in soup2.find_all('div', class_='ballon-video-player'):
            ...
            # for tag3 in soup2.find_all('ul',class_='download-activity-list'):
            #     tag3 = 'http://www.ghmba.online' + tag3["href"]
            #     shipin = tag3

            # 储存至数据库
            # value = ('真题精讲阶段', zjname, zjurl, shipin)
            # wsql.insert(value)
            print('本课程名称：真题精讲阶段 '+'课时：'+zjname+'课时URL：'+zjurl+'视频MP4地址：'+shipin)
